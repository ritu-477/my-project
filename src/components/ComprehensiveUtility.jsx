          ...
          <div className='lg:w-[558px] w-full flex flex-col max-lg:justify-center max-lg:items-center'>
            ...
            <div className="lg:mt-[46px] sm:mt-10 mt-8 lg:h-[462px]">
              {ACCORDION_DATA.map((item, index) => (
                <div
                  key={index}
                  className="bg-white max-w-[558px] !border-border-gray rounded-[4px] border mb-4 sm:mb-6 transition-all duration-500"
                >
                  <button
                    onClick={() => toggleAccordion(index)}
                    className={`w-full justify-between text-left text-deep-blue text-base sm:text-lg xl:text-2xl xl:leading-custom-4xl sm:p-[14px] p-3 font-normal transition-all duration-500 ${activeIndex === index
                      ? 'shadow-[0px_0px_13.9px_0px_#0000001A]' 
                        : '' 
                      }`}
                  >
